...
class Problem:
    ...
    def __init__(self, toolbox, data):
        ...
        self.__dict__.update({
            field: data[index] for field, index in fields.items()
        })
    ...
    def print(self, short=False, star=False):
        if not short:
            self.toolbox.console.print('Volume %d' % self.volume, bold=True)
        ...
        if short:
            if not self.history.accepted:
                ...
                self.toolbox.console.print(' %s ' % verdict[0],
                                           inv=True,
                                           end='')
            width = 49 - len(self.name)
            name = self.name[:49] + ' ' * width
            self.toolbox.console.alternate('', '*' if star else ' ', name,
                                           self.popularity,
        ...
        self.toolbox.console.alternate('Time limit',
                                       '%.0fs' % (self.time_limit / 1000),
                                       ' Best time',
                                       '%0.3fs' % (self.best_time / 1000),
                                       end='')
        if self.history.accepted:
            accepted = self.submissions.ac + self.submissions.pe
            self.toolbox.console.alternate(
                '  Your time ',
                '%.3fs' % (self.history.runtime / 1000),
                '  Rank ',
                str(self.history.rank),
                ' of %d (P≤' % accepted,
                '%.0f%%' % (100.0 * self.history.rank / accepted),
                ')',
                '',
                end='',
                sep='')
        ...
        self.toolbox.console.alternate(
            ...
            self.popularity,
            ')  AC ',
            '%.1f%%' % (self.percentage('ac') + self.percentage('pe')),
            '  (PE ',
            '%.0f%%' % self.percentage('pe'),
            ')  WA ',
            '%.0f%%' % self.percentage('wa'),
            '  TL ',
            '%.0f%%' % self.percentage('tl'),
            '  ML ',
            '%.0f%%' % self.percentage('ml'),
            '  ER ',
            '%.0f%%' % self.percentage('er'),
            sep='')
        self.toolbox.console.alternate('Expectation  AC ',
                                       '%.1f%%' % (100 * self.expected.ac),
                                       ' (',
                                       '%+.0f%%' % self.delta,
                                       ')  WA ',
                                       '%.0f%%' % (100 * self.expected.wa),
                                       '  TL ',
                                       '%.0f%%' % (100 * self.expected.tl),
                                       '  ML ',
                                       '%.0f%%' % (100 * self.expected.ml),
                                       '  Level ',
                                       self.DIFFICULTY[self.level],
                                       sep='')
...
class ProblemSet:
    ...
    def __init__(self, toolbox, data=None):
        ...
        for p in self.problems.values():
            ...
            avg_ac += (p.submissions.ac + p.submissions.pe) / max(
                1, p.non_errors)
        ...
        for p in self.problems.values():
            ...
            p.delta = round(
                p.percentage('ac') + p.percentage('pe') - 100 * p.expected.ac)
    ...
    def get_problem(self, *args, accept_none=False, ignore_current=False):
        if not args:
            ...
            if accept_none:
                return None
            raise Exception('no problem specified' if ignore_current else
                            'no problem currently selected')
        elif args[0] == '-':
